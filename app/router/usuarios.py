...
@router.get("/obtener-todos}", status_code=status.HTTP_200_OK, response_model=List[RetornarUsuario])
...
@router.get("/obtener-todos-secure", status_code=status.HTTP_200_OK, response_model=List[RetornarUsuario])
def get_all_s(
    db: Session = Depends(get_db),
    user_token: RetornarUsuario = Depends(get_current_user)
):
